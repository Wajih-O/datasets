...
def _write_tfrecord_from_shard_spec(shard_spec, get):
  ...
  tfds_writer._write_examples(shard_spec.path, itertools.chain(*iterators))
...
class ReaderTest(testing.TestCase):
  ...
  def _write_tfrecord(self, split_name, shards_number, records):
    ...
    with mock.patch.object(
        tfds_writer, '_get_number_shards', return_value=shards_number):
      shard_specs = tfds_writer._get_shard_specs(
